...
//		for (User user1 : listUser1) {
	...
	/*	}
		...
		Demo demo = new Demo();
		//System.out.println(demo.generateRandom(2));
		System.out.println("Single Digit " + randInt(1, 9));
	}		
	...
	public String addEmployee(User user){
	    ...
	    private String generateRandom(int number) {
			Random random = new Random();
			char[] digits = new char[number];
			digits[0] = (char) (random.nextInt(number-1) + '1');
			for (int i = 1; i < number; i++) {
				digits[i] = (char) (random.nextInt(number) + '0');
			}			
